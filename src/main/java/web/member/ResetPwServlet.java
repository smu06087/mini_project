...
//리셋패스워드->비밀번호 초기화(아이디,이메일 확인->새 비번1,2 일치->비번 업뎃)
...
public class ResetPwServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse res) throws IOException {
        ...
        newPw2 = (newPw2 == null) ? "" : newPw2.trim();
         ...
         LoginDAO logindao = LoginDAO.getInstance(); dao = MypageDAO.getInstance();
         // 1) 아이디+이메일이 실제로 존재하는지 확인
         boolean ok = logindao.verifyIdEmail(memberId, email);
         if (!ok) {
